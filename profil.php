...
// 1. Ambil data user yang sedang login berdasarkan session
...
if (isset($_POST['simpan'])) {
    ...
    // Logic Password: Jika kolom password diisi, update password. Jika kosong, pakai password lama.
    if (!empty($password_baru)) {
        // Cek apakah password di database di-encrypt md5. 
        // Jika tugas kuliah biasanya pakai md5(), ubah jadi: $password_final = md5($password_baru);
        $password_final = md5($password_baru); 
    } else {
        $password_final = $data['password']; // Tetap pakai password lama
    }

    // Logic Foto: Jika ada file baru diupload
    if ($foto_baru != '') {
        $cek_upload = upload_foto($_FILES["foto"]);
        if ($cek_upload['status']) {
            $foto_final = $cek_upload['message'];
            
            // Hapus foto lama jika ada (dan bukan foto default jika ada)
